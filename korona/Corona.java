...
class Person
{
    ...
    public Person(String prezime, String rezultat) {
    ...
    @Override
    public String toString() {
        return String.format("%s %s",prezime,rezultat);
    }
}
...
public class Corona {
    ...
    public static void main(String[] args) throws IOException {
        ...
        CBHT<String,Array<Person>> pozitivni = new CBHT<>(2*n);
        CBHT<String,Array<Person>> negativni = new CBHT<>(2*n);
        int counterPositive = 0,counterNegative = 0;
        for(int i = 0; i < n; i++)
        {
            String line = br.readLine();
            String [] parts = line.split(" ");
            Person person = new Person(parts[1],parts[2]);
            if(parts[2].equals("позитивен"))
            {
                if(pozitivni.search(parts[0]) == null)
                {
                    pozitivni.insert(parts[0], new Array<>(n));
                    pozitivni.search(parts[0]).element.value.insertLast(person);
                }
                else
                {
                    pozitivni.search(parts[0]).element.value.insertLast(person);
                ...
                if(parts[2].equals("негативен"))
                {
                    if(negativni.search(parts[0]) == null)
                    {
                        negativni.insert(parts[0], new Array<>(n));
                        negativni.search(parts[0]).element.value.insertLast(person);
                    }
                    else
                    {
                        negativni.search(parts[0]).element.value.insertLast(person);
        ...
        {
            counterPositive = pozitivni.search(opstina).element.value.getSize();
        }
        if(negativni.search(opstina)!= null)
        {
            counterNegative = negativni.search(opstina).element.value.getSize();
        }
        double rizikFaktor = counterPositive / (double) (counterNegative + counterPositive);
